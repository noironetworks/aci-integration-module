...
class NatStrategyMixin(NatStrategy):
    ...
    def delete_l3outside(self, ctx, l3outside, epg_name=None,
                         cidrs=None):
        if self._in_transaction(ctx.store.db_session):
            self._delete_l3out(ctx, l3outside, epg_name=epg_name,
                               cidrs=cidrs)
        else:
            with ctx.store.begin():
                self._delete_l3out(ctx, l3outside, epg_name=epg_name,
                                   cidrs=cidrs)
    ...
    def _create_l3out(self, ctx, l3out, vmm_domains=None, phys_domains=None,
        ...
        def create_l3out():
            ...
            self._create_nat_epg(ctx, l3out_db,
                                 ...
                                 epg_name=epg_name)
            return l3out_db
        if not self._in_transaction(ctx.store.db_session):
            with ctx.store.begin():
                return create_l3out()
        else:
            return create_l3out()
    ...
    def _create_ext_net(self, ctx, ext_net, provided_contracts=None,
        ...
        if not self._in_transaction(ctx.store.db_session):
            with ctx.store.begin():
                return create_ext_net()
        else:
            return create_ext_net()

    def _delete_ext_nets(self, ctx, ext_net,
                         provided_contracts=None,
                         consumed_contracts=None,
                         epg_name=None,
                         cidrs=None):
        ...
        if ext_net_db:
            if cidrs == None:
                ...
                self._delete_create_external_subnets(ctx, ext_net_db,
                                                     cidrs, True)
    ...
    def _delete_ext_net(self, ctx, ext_net,
                        ...
                        cidrs=None):
        if self._in_transaction(ctx.store.db_session):
            self._delete_ext_nets(ctx, ext_net,
                                  provided_contracts,
                                  consumed_contracts,
                                  epg_name, cidrs)
        else:
    ...
    def _delete_create_external_subnets(self, ctx, ext_net, cidrs, to_delete):
        ext_sub_attr = dict(tenant_name=ext_net.tenant_name,
                            l3out_name=ext_net.l3out_name,
                            external_network_name=ext_net.name)

        def delete_create_external_subnets():
            if to_delete:
                cur_ext_subs = self.mgr.find(ctx, resource.ExternalSubnet,
                                             **ext_sub_attr)
                for sub in cur_ext_subs:
                    if sub.cidr in cidrs:
                        self.mgr.delete(ctx, sub)
            else:
                for c in cidrs:
                    self.mgr.create(ctx, resource.ExternalSubnet(cidr=c,
        ...
        if not self._in_transaction(ctx.store.db_session):
            with ctx.store.begin():
                delete_create_external_subnets()
        else:
            delete_create_external_subnets()
    ...
    def _update_external_network_contract(self, ctx, ext_net,
                                          provided_contracts,
                                          consumed_contracts):
    ...
    def _update_external_network_contracts(self, ctx, ext_net,
                                           ...
                                           consumed_contracts):
        if self._in_transaction(ctx.store.db_session):
            self._update_external_network_contract(ctx, ext_net,
                                                   provided_contracts,
                                                   consumed_contracts)
        else:
            with ctx.store.begin():
...
class NoNatStrategy(NatStrategyMixin):
    ...
    def delete_external_networks(self, ctx, external_network,
                                 provided_contracts=None,
                                 consumed_contracts=None,
                                 epg_name=None, cidrs=None):
    ...
    def delete_external_network(self, ctx, external_network,
                                ...
                                epg_name=None, cidrs=None):
        if self._in_transaction(ctx.store.db_session):
            self.delete_external_networks(ctx, external_network,
                                          provided_contracts,
                                          consumed_contracts,
                                          epg_name, cidrs)
        else:
