...
class OwnershipManager(object):
    # TODO(ivar): this class should be split into 2, and the TenantManager
    # should call a factory method that returns whichever is more appropriate
    # based on ACI support.
    ...
    def __init__(self, apic_session, apic_config, aim_system_id):
        ...
        if self.use_annotation:
            self.owner_key = apic_config.get_option(
                'annotation_prefix', 'aim') + self.aim_system_id
        else:
            self.owner_key = self.aim_system_id
        self.owned_set = set()
        self.failure_log = {}
    ...
    def set_ownership_key(self, to_push):
        ...
        if self.use_annotation:
            for obj in to_push:
                obj.values()[0]['attributes']['annotation'] = self.owner_key
        else:
            tags = []
            for obj in to_push:
                if not obj.keys()[0].startswith(TAG_KEY):
                    dn = obj.values()[0]['attributes']['dn']
                    dn += '/tag-%s' % self.owner_key
        ...
        if self.use_annotation:
            for obj in to_push:
                if obj.keys()[0].startswith(TAG_KEY):
                    dn = obj.values()[0]['attributes']['dn']
                    if dn.endswith('/tag-%s' % self.aim_system_id):
        ...
        if self.use_annotation:
            for event in events:
                ...
                if AciTenantManager.is_deleting(event):
                    self.owned_set.discard(attrs['dn'])
                elif attrs.get('annotation') == self.owner_key:
                    self.owned_set.add(attrs['dn'])
            ...
            for event in events:
                if event.keys()[0] == TAG_KEY:
                    decomposed = event.values()[0][
                        'attributes']['dn'].split('/')
                    if decomposed[-1] == 'tag-' + self.owner_key:
                        parent_dn = '/'.join(decomposed[:-1])
                        if AciTenantManager.is_deleting(event):
                            self.owned_set.discard(parent_dn)
                        else:
                            self.owned_set.add(parent_dn)
                else:
                    managed.append(event)
            for event in managed:
                if AciTenantManager.is_deleting(event):
                    self.owned_set.discard(
    ...
    def is_owned(self, aci_object, check_parent=True):
        ...
        if type in apic_client.MULTI_PARENT:
            ...
            return '/'.join(decomposed[:-1]) in self.owned_set
        else:
            owned = dn in self.owned_set
            if not owned and AciTenantManager.is_child_object(
                ...
                return apic_client.DNManager().build(
                    decomposed[1][:-1]) in self.owned_set
            else:
                return owned

    def is_owned_dn(self, dn):
        return dn in self.owned_set
            ...
            for event in events:
                ...
                if (event.keys()[0] in [self.tenant.type, 'vmmDomP'] and not
                    ...
                    self._monitored_state = (
                        structured_tree.StructuredHashTree())
                    self.ownership_mgr.reset_ownership()
                    break
