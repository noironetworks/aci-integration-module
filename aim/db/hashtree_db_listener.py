        ...
        for root_rn in log_by_root:
            try:
                ...
                with ctx.store.begin(subtransactions=True):
                    ...
                    ttree_conf_db = htree.StructuredHashTree()
                    ttree_operational_db = htree.StructuredHashTree()
                    ttree_monitor_db = htree.StructuredHashTree()
                    ...
                    tree_map_db.setdefault(
                        self.tt_builder.CONFIG, {})[root_rn] = ttree_conf_db
                    tree_map_db.setdefault(
                        self.tt_builder.OPER, {})[root_rn] = (
                        ttree_operational_db)
                    tree_map_db.setdefault(
                        self.tt_builder.MONITOR, {})[root_rn] = (
                        ttree_monitor_db)
                    ...
                    for action, aim_res, _ in log_by_root[root_rn]:
                        ...
                        self.tt_builder.build(added, [], deleted,
                                              ...
                                              aim_ctx=ctx)
                        # Build the hashmap for DB.
                        self.tt_builder.build(added, [], deleted,
                                              tree_map_db, inmem=False,
                                              aim_ctx=ctx)
